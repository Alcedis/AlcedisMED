...
class RKrExportCase
{
    ...
    public function setCreateTime($create_time, $update_childern = false )
    {
        ...
        if ($update_childern ) {
            ...
            foreach($this->m_sections as $section ) {
                if ($section instanceof RKrExportSection ) {
     ...
     *
     * @access  public
     * @param   RKrExportSection[] $sections
     * @return  RKrExportCase
     */
     ...
     *
     * @access  public
     * @param   RKrExportSection $section
     * @return  RKrExportCase
    ...
    public function Delete($db, $export_case_id = 0)
    {
        ...
        if (0 != $dbid) {
            // Delete object
            $queries = array(
                "DELETE FROM export_case_log WHERE export_case_log_id = '{$dbid}'",
                "DELETE FROM export_section_log WHERE export_case_log_id = '{$dbid}'"
            );
    ...
    public function getAllInvalidSections()
    {
        ...
        foreach( $this->m_sections as $section) {
            if (( $section instanceof RKrExportSection) && (0 == $section->getValid())) {
                $arr = $section->ToArray();
    ...
    public function getAllSectionsWithOnlyWarnings()
    {
        ...
        foreach( $this->m_sections as $section) {
            if (($section instanceof RKrExportSection) && (1 == $this->HasDataChanged()) && $section->_hasOnlyWarnings()) {
                $arr = $section->ToArray();
                $arr[ 'patient_id' ]     = $this->m_patient_id;
                $arr[ 'erkrankung_id' ]  = $this->m_erkrankung_id;
                $arr[ 'diagnose_seite' ] = $this->m_diagnose_seite;
    ...
    public function setSectionErrorsByName($blockName, $errors)
    {
        ...
        throw new EKrExportException("ERROR: Section [$blockName] not found.");
    }
    ...
    public function setSectionErrorsByUid( $sectionUid, $errors)
    {
        ...
        throw new EKrExportException("ERROR: Section UID [$sectionUid] not found.");
    }
    ...
    public function setSectionTanByUid( $section_uid, $tan)
    {
        ...
        throw new EKrExportException( "ERROR: Section UID [$section_uid] not found.");
    }
     ...
     * @return  void
     */
    public static function addToQueryStack($values)
    {
