...
require_once( 'class.krbw_ng_utils_1_0_0.php' );

class CKrbwNg1_0_0 extends CMedBaseExport
{
   ...
   protected function Init()
   {
      ...
      $this->_smarty->config_load( 'app/export_krbw_ng.conf', 'export_krbw_ng' );
      $this->_smarty->config_load(FILE_CONFIG_APP);
      $this->_config = $this->_smarty->get_config_vars();

      $this->_xml_template = array(
		 'diagnose'  => 'app/xml.export_krbw_ng_diagnose_1_0_0.tpl',
         'verlauf'   => 'app/xml.export_krbw_ng_verlauf_1_0_0.tpl',
         'therapie'  => 'app/xml.export_krbw_ng_therapie_1_0_0.tpl',
         'abschluss' => 'app/xml.export_krbw_ng_abschluss_1_0_0.tpl'
      );
   }
   ...
   protected function CreateExportFilter( $session, $request )
   {
      ...
      $ext_dir = isset( $this->_config[ 'exp_krbw_ng_dir' ] ) ? $this->_config[ 'exp_krbw_ng_dir' ] : 'krbw_ng/';
      $ext_log_subdir = isset( $this->_config[ 'exp_krbw_ng_log_subdir' ] ) ? $this->_config[ 'exp_krbw_ng_log_subdir' ] : 'log/';
      $ext_tmp_subdir = isset( $this->_config[ 'exp_krbw_ng_tmp_subdir' ] ) ? $this->_config[ 'exp_krbw_ng_tmp_subdir' ] : 'tmp/';
      $ext_xml_subdir = isset( $this->_config[ 'exp_krbw_ng_xml_subdir' ] ) ? $this->_config[ 'exp_krbw_ng_xml_subdir' ] : 'xml/';
      $ext_zip_subdir = isset( $this->_config[ 'exp_krbw_ng_zip_subdir' ] ) ? $this->_config[ 'exp_krbw_ng_zip_subdir' ] : 'zip/';
   ...
   {
      $filename = $this->_xml_dir . 'krbw_ng_export_' . date( 'YmdHis' );
      return $filename;
   }
   ...
   protected function WriteContent( $content, $filename, $export_filter )
   {
      ...
      $xml_file = $filename;
      $zip_file = $this->_zip_dir . 'krbw_ng_export_files_' . date( 'YmdHis' ) . ".zip";

      $zip = new PclZip( $zip_file );
      $zip_create = $zip->create( $files, PCLZIP_OPT_REMOVE_ALL_PATH );

      $zip_url = "index.php?page=export_krbw_ng&action=download&type=zip&file=" . $zip_file;
   ...
   protected function GetAuthorDaten( $ekr_id, $export_filter )
   {
      ...
      $quell_system = isset( $this->_config[ 'exp_krbw_ng_source_system' ] ) ? $this->_config[ 'exp_krbw_ng_source_system' ] : '0';
   ...
   protected function GetMelderDaten( $ekr_id, $export_filter )
   {
      ...
      $melder_id = isset( $this->_config[ 'exp_krbw_ng_reporter_id' ] ) ? $this->_config[ 'exp_krbw_ng_reporter_id' ] : '0';
  ...
  /**
   ...
   protected function GetAuthorIdentDaten( $ekr_id, $export_filter )
   {
      ...
      $absender_melder_id = isset( $this->_config[ 'exp_krbw_ng_despatch_reporter_id' ] ) ? $this->_config[ 'exp_krbw_ng_despatch_reporter_id' ] : '0';
