...
class CWbc2010_7_0 extends CMedBaseExport
{
   ...
   {
      $this->_schema_file = 'feature/exports/scripts/wbc/WBCExport_Schema_2010v7.xsd';
      $this->_xml_template = 'app/xml.export_wbc_2010_7.tpl';
      $this->_smarty->config_load( 'settings/interfaces.conf' );
      $this->_smarty->config_load( 'app/export_wbc.conf', 'export_wbc' );
   ...
   protected function CreateExportFilter( $session, $request )
   {
      ...
      $ext_wbc_dir = isset( $this->_config[ 'exp_wbc_dir' ] ) ? $this->_config[ 'exp_wbc_dir' ] : 'wbc/';
      $ext_wbc_log_subdir = isset( $this->_config[ 'exp_wbc_log_subdir' ] ) ? $this->_config[ 'exp_wbc_log_subdir' ] : 'log/';
      $ext_wbc_tmp_subdir = isset( $this->_config[ 'exp_wbc_tmp_subdir' ] ) ? $this->_config[ 'exp_wbc_tmp_subdir' ] : 'tmp/';
      $ext_wbc_xml_subdir = isset( $this->_config[ 'exp_wbc_xml_subdir' ] ) ? $this->_config[ 'exp_wbc_xml_subdir' ] : 'xml/';
      $ext_wbc_zip_subdir = isset( $this->_config[ 'exp_wbc_zip_subdir' ] ) ? $this->_config[ 'exp_wbc_zip_subdir' ] : 'zip/';
      $this->_export_path = $this->GetExportPath( $ext_wbc_dir, $export_filter[ 'login_name' ] );
      if ( file_exists( $this->_export_path ) ) {
         $this->DeleteDirectory( $this->_export_path );
      }
      $this->_log_dir = $this->_export_path . $ext_wbc_log_subdir;
      $this->_tmp_dir = $this->_export_path . $ext_wbc_tmp_subdir;
      $this->_xml_dir = $this->_export_path . $ext_wbc_xml_subdir;
      $this->_zip_dir = $this->_export_path . $ext_wbc_zip_subdir;
      // Pfade anlegen
      ...
      $export_all_data = isset( $request[ 'use_all_data' ] ) ? $request[ 'use_all_data' ] : '0';
      // Formular Daten holen
      if ( $export_all_data == '0' ) {
         $year = isset( $request[ 'year' ] ) ? $request[ 'year' ] : '';
         $export_filter[ 'von' ] = ( $year - 1 ) . "-01-01";
         $export_filter[ 'bis' ] = "$year-12-31";
      }
      else {
         $export_filter[ 'von' ] = "1900-01-01";
         $export_filter[ 'bis' ] = date( 'Y-m-d' );
      }
      // export_id bestimmen
      $export_filter[ 'export_id' ] = dlookup( $this->_db, 'exp_wbc_log', 'IFNULL( MAX( export_id ) + 1, 1 )', '1' );
   ...
   {
      $filename = $this->_xml_dir . 'wbc_export_' . date( 'YmdHis' ) . '.xml';
      return $filename;
   }
   ...
   protected function WriteContent( $content, $filename, $export_filter )
   {
      ...
      if ( count( $content[ 'valid' ] ) > 0 ) {
         ...
         $zip_url = "index.php?page=export_wbc&action=download&type=zip&file=" . $zip_file;
      }
      $result = array( 'valid' => array(),
                       'invalid' => array() );
   ...
   {
      $datenexport = array();
      $datenexport[ 'klinik_id' ] = isset( $this->_config[ 'exp_wbc_klinik_id' ] ) ? $this->_config[ 'exp_wbc_klinik_id' ] : '';
      ...
      $ansprechpartner[ 'name' ] = $this->_config[ 'exp_wbc_ansprechpartner_name' ];
      $ansprechpartner[ 'mail' ] = $this->_config[ 'exp_wbc_ansprechpartner_email' ];
   ...
   protected function ExtractData( $export_filter )
   {
      ...
      $query = "
         SELECT
           ...
           ts.erkrankung_id,
           ts.diagnose_seite,
           ts.datum_sicherung        AS 'db_ts',
           MIN( h.datum )            AS 'db_h'

         FROM
           patient p
           INNER JOIN erkrankung e   ON e.erkrankung='b'
                                        AND e.patient_id = p.patient_id
           INNER JOIN tumorstatus ts ON e.erkrankung_id=ts.erkrankung_id
                                        AND ts.anlass = 'p'
         ...
         HAVING
           ( db_ts BETWEEN '{$export_filter[ 'von' ]}' AND '{$export_filter[ 'bis' ]}' )
           OR
           ( ( db_h IS NOT NULL ) AND ( db_h BETWEEN '{$export_filter[ 'von' ]}' AND '{$export_filter[ 'bis' ]}' ) )
      ";
      */
      /*
      ...
      $query = "
         ...
         FROM
           ...
           INNER JOIN ( SELECT
                        ...
                        ORDER BY
                          ts_i.datum_sicherung DESC,
                          ts_i.sicherungsgrad ASC,
                          ts_i.datum_beurteilung DESC ) ts ON e.erkrankung_id=ts.erkrankung_id
           LEFT JOIN histologie h                          ON h.erkrankung_id=ts.erkrankung_id
                                                              AND h.diagnose_seite=ts.diagnose_seite
      ...
      */
      ...
      if ( ( $result !== false ) && ( count( $result ) > 0 ) ) {
         ...
         foreach( $tumorstatus as $ts ) {
            $insert = false;
            if ( ( strtotime( $ts[ 'datum_sicherung' ] ) >= strtotime( "{$export_filter[ 'von' ]}" ) ) &&
                 ( strtotime( $ts[ 'datum_sicherung' ] ) <= strtotime( "{$export_filter[ 'bis' ]}" ) ) ) {
                $insert = true;
            }
            else {
                if ( $this->HasTsHistologieInRange( $ts, $export_filter ) ) {
                    $insert = true;
                }
            }
            if ( $insert ) {
   ...
   protected function HasTsHistologieInRange( $ts, $export_filter ) {
      $query = "
         SELECT
           *
         ...
         WHERE
           h.erkrankung_id={$ts[ 'erkrankung_id' ]}
					 AND h.diagnose_seite='{$ts[ 'diagnose_seite' ]}'
      ...
      $result = end( sql_query_array( $this->_db, $query ) );
      if ( $result !== false ) {
          if ( ( strtotime( $result[ 'datum' ] ) >= strtotime( "{$export_filter[ 'von' ]}" ) ) &&
               ( strtotime( $result[ 'datum' ] ) <= strtotime( "{$export_filter[ 'bis' ]}" ) ) ) {
             return true;
         }
      }
      return false;
   ...
   protected function GetPatientData( $patient_id )
   {
      $query = "
         SELECT
            ...
            IFNULL( geschlecht.code_wbc, 'x' )              AS geschlecht,
            DATE_FORMAT( abschluss.todesdatum, '%Y-%m-%d' ) AS todesdatum

         FROM
            patient p
            LEFT JOIN l_exp_wbc geschlecht ON p.geschlecht=geschlecht.code_med
   ...
   protected function GetCaseData( $tumorstatus_id )
   {
      $query = "
         ...
         FROM
            tumorstatus ts
            INNER JOIN l_exp_wbc seite ON ts.diagnose_seite=seite.code_med
   ...
   protected function GetDiagnosen( $data, $export_filter )
   {
      ...
      $query = "
		 (
         	 SELECT
               ...
               CASE
                  WHEN MAX( eoe.prozedur ) IS NOT NULL THEN '1'
                  WHEN MAX( eof.prozedur ) IS NOT NULL THEN '2'
                  WHEN MAX( eog.prozedur ) IS NOT NULL THEN '4'
                  ELSE '6'
               END                                                AS biopsie_bildgebende_kontrolle,
            ...
            FROM
               ...
               LEFT JOIN eingriff_ops eoe ON eoe.eingriff_id=e.eingriff_id
                                             AND eoe.prozedur IN ( '1-e03.4', '1-e03.5', '1-e03.6', '1-e03.7', '1-e03.8' )
               LEFT JOIN eingriff_ops eof ON eof.eingriff_id=e.eingriff_id
                                             AND eof.prozedur IN ( '1-e03.9', '1-e03.10', '1-e03.11', '1-e03.12', '1-e03.13' )
               LEFT JOIN eingriff_ops eog ON eog.eingriff_id=e.eingriff_id
                                             AND eog.prozedur IN ( '1-e03.14', '1-e03.15', '1-e03.16', '1-e03.17', '1-e03.18' )
            WHERE
            ...
            GROUP BY
         	...
         	SELECT
               '7'                                                AS biopsie_methode,
               '6'                                                AS biopsie_bildgebende_kontrolle,
      ...
      if ( $result !== false ) {
         ...
         $diagnosen[ 0 ][ 'biopsie_datum' ] = '1900-01-01';
      }
      ...
      $query = "
         ...
         FROM
            tumorstatus ts
            LEFT JOIN l_exp_wbc ct ON ts.t=ct.code_med
                                       AND ct.klasse='ct'
			LEFT JOIN l_exp_wbc cn ON ts.n=cn.code_med
                                       AND cn.klasse='cn'
			LEFT JOIN l_exp_wbc cm ON ts.m=cm.code_med
                                       AND cm.klasse='cm'
   ...
   protected function GetTherapieOperations( $data )
   {
      $query = "
         SELECT
            ...
            IFNULL(mark.code_wbc, '3')                     AS praeoperative_markierung,
            CASE
               WHEN e.postop_roentgen='1'
               		OR e.intraop_roe='1'       THEN '1'
               WHEN e.postop_roentgen='0'
               		OR e.intraop_roe='0'       THEN '0'
               ELSE                                 '2'
            END                                            AS praeparateroentgen,
            IFNULL(h.resektionsrand, -2)                   AS operativer_sicherheitsabstand,
            IFNULL(r.code_wbc, r_lokal.code_wbc)           AS lokale_pathohisto_radikalitaet,
            CASE
         ...
         FROM
            ...
            LEFT JOIN l_exp_wbc mark    ON e.mark=mark.code_med
                                           AND mark.klasse='praeoperative_markierung'
            LEFT JOIN l_exp_wbc r       ON tsp.r=r.code_med
                                           AND r.klasse='r'
            LEFT JOIN l_exp_wbc r_lokal ON tsp.r_lokal=r_lokal.code_med
   ...
   protected function GetTherapieSystemtherapien( $data )
   {
      $query = "
		(
    		SELECT
    			IFNULL(systemtherapie.code_wbc, '4')	AS systemtherapie,
                IF(t.ende IS NOT NULL, '3', '2')    	AS systemtherapie_ausfuehrung,
                systemtherapie_intention.code_wbc   	AS systemtherapie_intention,
    		...
    		FROM
    			...
    			LEFT JOIN l_exp_wbc systemtherapie              ON systemtherapie.klasse='systemtherapie'
    			                                                    AND t.vorlage_therapie_art=systemtherapie.code_med
    			LEFT JOIN l_exp_wbc systemtherapie_intention    ON systemtherapie_intention.klasse='systemtherapie_intention'
   ...
   protected function GetHistologien( $data )
   {
      ...
      if ( ( $result !== false ) && ( count( $result ) > 0 ) ) {
         ...
         $histologien[ 0 ][ 'patho_histo_klassifikationen' ] = array();
         $histo = $this->GetHistologienPhk( $data );
         if ( ( count( $histo ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'morpho_code' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'topologie_code' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'histo_datum' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'icdo_version' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 't' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'n' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'm' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'y' ] ) > 0 ) &&
              ( strlen( $histo[ 0 ][ 'tnm_version' ] ) > 0 ) ) {
             $histologien[ 0 ][ 'patho_histo_klassifikationen' ] = $histo;
         }
      }
      return $histologien;
   }
      ...
      $query = "
         SELECT DISTINCT
            ...
            IF(
         	...
         	m.code_wbc                  AS m,
         	IF(
         	    t.code_wbc IS NOT NULL
         	    OR n.code_wbc IS NOT NULL
         	    OR m.code_wbc IS NOT NULL
         	,
         	    IFNULL(y.code_wbc, '0')
         	,
         		NULL
         ...
         FROM
         	tumorstatus ts
         	LEFT JOIN l_exp_wbc t   ON t.klasse='pt'
         	                            AND ts.t=t.code_med
         	LEFT JOIN l_exp_wbc n   ON n.klasse='pn'
         	                            AND ts.n=n.code_med
         	LEFT JOIN l_exp_wbc m   ON m.klasse IN ('cm', 'pm')
         	                            AND ts.m=m.code_med
         	LEFT JOIN l_exp_wbc y   ON y.klasse='y'
   ...
   protected function GetLabore( $item )
   {
      ...
      $query = "
         SELECT
           	IFNULL(er.code_wbc, er2.code_wbc)		AS er,
         	'2'                                     AS er_score

        FROM
            tumorstatus ts
			LEFT JOIN l_exp_wbc	er	ON er.klasse='irs'
										AND ts.estro_irs=er.code_med
			LEFT JOIN l_exp_wbc	er2	ON er2.klasse='rez_pn'
      ...
      $query = "
         SELECT
           IFNULL(pr.code_wbc, pr2.code_wbc)		AS pr,
           '2'                                      AS pr_score

        FROM
            tumorstatus ts
			LEFT JOIN l_exp_wbc	pr	ON pr.klasse='irs'
										AND ts.prog_irs=pr.code_med
			LEFT JOIN l_exp_wbc	pr2	ON pr2.klasse='rez_pn'
