...
require_once('class.dmp_2014_0_serialiser.php');
require_once('class.dmp_2014_0_section.php');
require_once('class.dmp_2014_0_observation.php');
require_once('feature/export/helper.dmp.php');

class Cdmp_2014_0_Model extends CExportDefaultModel
{
    ...
    public function ExtractDataExt($parameters, $wrapper, &$exportRecord, $id = -1, $type = '' ) {
        ...
        if ($type != '') {
            $this->m_l_basic = HDatabase::ReadExportCodeTable($this->m_db, 'dmp');
        }
        $functionNames = array(
            '_CreateEdSection',
            '_CreateEdPnpSection',
        ...
        if (($type == '') || ($type == 'ed')) {
            ...
            $query = "
                SELECT
                    ...
                    p.adelstitel                           AS patient_adelstitle,
                    p.strasse                              AS patient_strasse,
                    p.hausnr                               AS patient_hausnr,
                    p.adrzusatz                            AS patient_adrzusatz,
                    p.plz                                  AS patient_plz,
                    p.ort                                  AS patient_ort,
                    p.staat                                AS patient_staat,
                    p.geburtsdatum                         AS patient_geburtsdatum,
                    p.geschlecht                           AS patient_geschlecht,
                    p.kv_nr                                AS patient_versich_nr,
                    p.kv_status                            AS patient_kv_versichertenstatus,
                    p.kv_wop                               AS patient_kv_wop,
                    p.kv_besondere_personengruppe          AS patient_kv_besondere_personengruppe,
                    p.kv_dmp_kennzeichnung                 AS patient_kv_dmp_kennzeichnung,
                    p.kv_versicherungsschutz_beginn        AS patient_kv_versicherungsschutz_beginn,
                    p.kv_versicherungsschutz_ende          AS patient_kv_versicherungsschutz_ende,
                    ...
                    ktst.vknr_ref                          AS krankenkasse_vknr_ref,
                    ...
                    u.adelstitel                           AS arzt_adelstitle,
    ...
    public function CheckData($parameters, &$exportRecord)
    {
        ...
        $serialiser = new Cdmp_2014_0_Serialiser;
        $serialiser->Create(
    ...
    protected function _CreateHeaderSection($parameters, $extractData, &$sectionUid)
    {
        ...
        $header['provider']['arzt']['vorname'] = HCommon::TrimString($extractData['arzt_vorname'], 45, true);
        $header['provider']['arzt']['nachname'] = HCommon::TrimString($extractData['arzt_nachname'], 45, true);
        $header['provider']['arzt']['titel_ac'] = HCommon::TrimString($extractData['arzt_title'], 20, true);
        $header['provider']['arzt']['titel_nb'] = HCommon::TrimString($extractData['arzt_adelstitle'], 20, true);
        ...
        $header['provider']['org']['name'] =  HCommon::TrimString($extractData['org_name'], 40, true);
        $header['provider']['org']['fachabteilung'] =  HCommon::TrimString($extractData['fachabteilung'], 40, true);
        $header['provider']['org']['strasse'] =  HCommon::TrimString($extractData['org_strasse'], 46, true);
        $header['provider']['org']['hausnr'] =  HCommon::TrimString($extractData['org_hausnr'], 9, true);
        $header['provider']['org']['plz'] =  HCommon::TrimString($extractData['org_plz'], 10, true);
        $header['provider']['org']['ort'] =  HCommon::TrimString($extractData['org_ort'], 40, true);
        $header['provider']['org']['postfach'] = HCommon::TrimString('', 8, true);;
        $header['provider']['org']['cnt'] = HCommon::TrimString($extractData['org_staat'], 3, true);
        // Krankenhaus-IK
        ...
        $header['provider']['patient']['vorname'] = HCommon::TrimString($extractData['patient_vorname'], 45, true);
        $header['provider']['patient']['nachname'] = HCommon::TrimString($extractData['patient_nachname'], 45, true);
        $header['provider']['patient']['titel_ac'] = HCommon::TrimString($extractData['patient_title'], 20, true);
        $header['provider']['patient']['titel_nb'] = HCommon::TrimString($extractData['patient_adelstitle'], 20, true);
        $header['provider']['patient']['strasse'] =  HCommon::TrimString($extractData['patient_strasse'], 46, true);
        $header['provider']['patient']['hausnr'] =  HCommon::TrimString($extractData['patient_hausnr'], 9, true);
        $header['provider']['patient']['adrzusatz'] =  HCommon::TrimString($extractData['patient_adrzusatz'], 40, true);
        $header['provider']['patient']['plz'] =  HCommon::TrimString($extractData['patient_plz'], 10, true);
        $header['provider']['patient']['ort'] =  HCommon::TrimString($extractData['patient_ort'], 40, true);
        $header['provider']['patient']['postfach'] =  '';
        $header['provider']['patient']['cnt'] =  HCommon::TrimString($extractData['patient_staat'], 3, true);
        ...
        $header['provider']['patient']['kv_versichertenstatus'] = $extractData['patient_kv_versichertenstatus'];
        $header['provider']['patient']['kv_wop'] = $extractData['patient_kv_wop'];
        $header['provider']['patient']['besonderePersonengruppe'] = $extractData['patient_kv_besondere_personengruppe'];
        $header['provider']['patient']['dmp_kennzeichnung'] = $extractData['patient_kv_dmp_kennzeichnung'];
        $header['provider']['patient']['kv_versicherungsschutz_beginn'] = $this->_CheckDatum($extractData['patient_kv_versicherungsschutz_beginn']);
        $header['provider']['patient']['kv_versicherungsschutz_ende'] = $this->_CheckDatum($extractData['patient_kv_versicherungsschutz_ende']);
        $header['provider']['patient']['kv_einlesedatum'] = $this->_CheckDatum($extractData['patient_kvk_einlesedatum']);
        ...
        $header['provider']['krankenkasse'] = array(
            ...
            'vknr'                  => (strlen($extractData['krankenkasse_vknr_ref']) > 0) ? $extractData['krankenkasse_vknr_ref'] : $extractData['krankenkasse_vknr'],
    ...
    protected function _FillErstmeldungsSections($extractData, &$sections)
    {
        ...
        if (isset($extractData['mani_primaer']) ||
            ...
            $section = new Cdmp_2014_0_Section;
            ...
            if (strlen($extractData['mani_primaer']) > 0) {
                $observation = new Cdmp_2014_0_Observation;
