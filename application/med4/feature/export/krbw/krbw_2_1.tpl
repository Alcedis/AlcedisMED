          ...
          <DIAGNOSE>
            ...
            <KLR>
              ...
              <METASTASEN>
	              ...
	              <METASTASE>
	             		<METASTASENLOKALISATION>{$metastase}</METASTASENLOKALISATION>
	             	</METASTASE>
	              {/foreach}
              </METASTASEN>
              {/if}
        ...
        <MELDUNG>
          <VERLAUF>
            ...
            <KLR>
              ...
              {if strlen($verlauf.t) > 0 ||
                  strlen($verlauf.n) > 0 ||
                  strlen($verlauf.m) > 0}
              <TNM_VERSION>{$verlauf.tnm_version}</TNM_VERSION>
              {if strlen($verlauf.t) > 0}
              <T_STADIUM>{$verlauf.t}</T_STADIUM>
              {/if}
              {if strlen($verlauf.n) > 0}
              <N_STADIUM>{$verlauf.n}</N_STADIUM>
              {/if}
              {if strlen($verlauf.m) > 0}
              <M_STADIUM>{$verlauf.m}</M_STADIUM>
              {/if}
              {/if}
              {if isset($verlauf.metastasen) && count($verlauf.metastasen) > 0}
              <METASTASEN>
                {foreach from=$verlauf.metastasen item=metastase }
                    <METASTASE>
                        <METASTASENLOKALISATION>{$metastase.metastasenlokalisation}</METASTASENLOKALISATION>
                        <METASTASE_DIAGNOSEDATUM>{$metastase.metastase_diagnosedatum}</METASTASE_DIAGNOSEDATUM>
                    </METASTASE>
                {/foreach}
              </METASTASEN>
            {/if}
