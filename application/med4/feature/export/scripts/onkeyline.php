...
if ($parameter['melder_id'] === null) {
    $where = "export_name = 'onkeyline' AND finished = '0' AND export_unique_id LIKE '{$parameter['user_id']}-%'";
    ...
    $exportUniqueId = "{$parameter['user_id']}-{$parameter['melder_id']}";
}
...
if ( ( strlen( $parameter[ 'org_id' ] ) > 0 ) &&
    ...
    $export = CExportFactory::CreateObject( $smarty, $db, 'onkeyline', '1.0', 'ext_err' );
