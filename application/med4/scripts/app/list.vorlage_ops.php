...
$searchFields = array(
   'code'  => array('type' => 'string', 'field'   => "vo.code"),
   'bez'   => array('type' => 'string', 'field'   => "vo.bez"),
);

$queryMod = queryModifier::create($db, $smarty)
   ->setCookie($cookie)
   ->setQuery($querys['vorlage_ops'])
   ->setSearchFields($searchFields)
   ->setOrderBy('vo.code ASC')
;

data2list($db, $fields, $queryMod->query());

$arr_menubar['vorlage_ops'] = array('new' => ($permission->action('I') === true));
