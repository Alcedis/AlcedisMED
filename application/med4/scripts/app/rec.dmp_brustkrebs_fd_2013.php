...
$table    = 'dmp_brustkrebs_fd_2013';
$formId   = isset($_REQUEST['dmp_brustkrebs_fd_2013_id']) ? $_REQUEST['dmp_brustkrebs_fd_2013_id'] : '';
$dmpEdId  = isset($_REQUEST['dmp_brustkrebs_ed_2013_id']) ? $_REQUEST['dmp_brustkrebs_ed_2013_id'] : '';
$location = get_url('page=view.erkrankung');

if (strlen($dmpEdId) > 0) {
    $_REQUEST['einschreibung_grund'] = dlookup(
        $db,
        'dmp_brustkrebs_ed_2013',
        'einschreibung_grund',
        "dmp_brustkrebs_ed_2013_id={$dmpEdId}"
    );
}
...
function ext_warn($valid)
{
    ...
    if (isset($_SESSION['dmp_xml_protokoll'])) {
        ...
        unset($_SESSION['dmp_xml_protokoll']);
    } elseif (isset($_REQUEST['dmp_brustkrebs_fd_2013_id']) &&
        (strlen($_REQUEST['dmp_brustkrebs_fd_2013_id']) > 0)) {
        $db = $valid->_db;
        $dmpId = $valid->_fields['dmp_brustkrebs_fd_2013_id']['value'][0];
        $dmpProtocol = dlookup($db, 'dmp_brustkrebs_fd_2013', 'xml_protokoll', "dmp_brustkrebs_fd_2013_id=$dmpId");
    }
    HelperDmp::showDmpErrors($valid->_db, "fd", 0, $dmpProtocol, $valid);
}
?>
