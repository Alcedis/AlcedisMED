...
$table      = 'qs_18_1_o';
$form_id    = isset( $_REQUEST['qs_18_1_o_id'] ) ? $_REQUEST['qs_18_1_o_id'] : '';
$bId        = isset( $_REQUEST['qs_18_1_b_id'] ) ? $_REQUEST['qs_18_1_b_id'] : dlookup($db, 'qs_18_1_o', 'qs_18_1_b_id', "qs_18_1_o_id = '{$form_id}'");
...
//Nach 2009 nicht mehr gültig
if ($qs181Converter->getVersion() > 2009) {
    unset($fields['diagoffbiopsie']);
    $smarty->widget->unsetField('diagoffbiopsie');
}
...
function ext_err($valid)
{
    ...
    $qs181brustId  = reset($fields['qs_18_1_brust_id']['value']);

    $errors = qs181ValidationParser::create($db)
        ->setLayer('o')
        ->setFields(array(
                'b'     => reset(sql_query_array($db, "SELECT * FROM qs_18_1_b WHERE qs_18_1_b_id = '{$qs181bId}'")),
                'brust' => reset(sql_query_array($db, "SELECT * FROM qs_18_1_brust WHERE qs_18_1_brust_id = '{$qs181brustId}'")),
                'o'     => fields2dataArray($fields)
        ))
        ->parse()
        ->getErrors()
    ;
