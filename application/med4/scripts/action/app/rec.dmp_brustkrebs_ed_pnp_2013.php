...
switch ($action) {
    case 'insert':
        ...
        if (appSettings::get('interfaces', null, 'dmp') === true) {
            ...
            if ($noErrors) {
                $dokuDate  = reset($fields['doku_datum']['value']);
                $patientId = reset($fields['patient_id']['value']);
                $eg        = reset($fields['einschreibung_grund']['value']);

                $formId  = dlookup(
                    $db,
                    'dmp_brustkrebs_ed_pnp_2013',
                    'dmp_brustkrebs_ed_pnp_2013_id',
                    "doku_datum='{$dokuDate}' AND einschreibung_grund = '{$eg}' AND patient_id='{$patient_id}'"
                );
                ...
                $result = HelperDmp::checkForm($parameters, $formId, 'ed_pnp', $smarty, $db, $_REQUEST['doku_datum']);

                // Ergebnis des Checks in die DB schreiben
                HelperDmp::setXml($result, $_REQUEST, $_SESSION);

                $_REQUEST['dmp_brustkrebs_ed_pnp_2013_id'] = $formId;
    ...
    case 'get_dmp':
        ...
        dmp2013Preallocate::create($db, 'pnp', $param)
