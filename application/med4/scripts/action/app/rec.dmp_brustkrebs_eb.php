...
$dmp_ed_obj    = CDmpFactory::createObject( 'Ed', '2.22', $smarty, $db );
...
//         if ( $no_errors && !is_null( $dmp_ed_obj ) ) {
//            if ( strlen( $_REQUEST[ 'fall_nr'] ) ) {
//               $query = "UPDATE dmp_nummern SET patient_id='$patient_id' WHERE org_id='$org_id' AND nr='" . $_REQUEST['fall_nr'] . "'";
...
//               $dmp_db_id  = dlookup( $db, 'dmp_brustkrebs_eb', 'dmp_brustkrebs_eb_id', "doku_datum='" . $doku_datum_en . "' AND einschreibung_grund='" . $_REQUEST[ 'einschreibung_grund' ] . "' AND patient_id=" . $_REQUEST[ 'patient_id' ] );
//               $dmp_ed_obj->generate( $dmp_db_id );
//               $_REQUEST['dmp_brustkrebs_eb_id'] = $dmp_db_id;
//               setXml( $dmp_ed_obj, $_REQUEST, $_SESSION );
...
//      }
   ...
   case 'update':
      ...
      if ( $no_errors && !is_null( $dmp_ed_obj ) ) {
         $errors = $smarty->get_template_vars( 'error' );
         if ( is_null( $errors ) || ( strlen( $errors ) > 0 ) ) {
            $dmp_ed_obj->generate( $form_id );
            setXml( $dmp_ed_obj, $_REQUEST, $_SESSION );
            $_REQUEST['dmp_brustkrebs_eb_id'] = $form_id;
